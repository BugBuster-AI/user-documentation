...
-  **Когда нужно ввести актуальную дату или время в поле** -- например, дату завершения задачи, время встречи, период действия документа. Переменная подставит текущее значение, и вам не придётся каждый раз обновлять тест-кейс вручную.

-  **Когда нужно проверить состояние приложения, которое зависит от текущей даты** -- например, убедиться, что задача отображается с правильным дедлайном, или что в списке показываются события на сегодня.
