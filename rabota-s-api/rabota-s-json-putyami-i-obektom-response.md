...
| `response.url`        | Итоговый URL запроса после редиректов           |
...
## 4\. Основные правила обращения к данным
...
| Что получить       | Какой путь                                                                            |
|--------------------|---------------------------------------------------------------------------------------|
| ID пользователя    | `{{`[`response.body.user.id`](http://response.body.user.id)`}}`                       |
| Email пользователя | `{{`[`response.body.user.profile.email`](http://response.body.user.profile.email)`}}` |
...
## 6\. Использование в Variables

| Variable Name | Response Path                                                                         |
|---------------|---------------------------------------------------------------------------------------|
| `token`       | `{{response.body.token}}`                                                             |
| `email`       | `{{`[`response.body.user.profile.email`](http://response.body.user.profile.email)`}}` |
| `role`        | `{{response.body.roles[0].name}}`                                                     |

---

## 7\. Использование в Validations

| Target                                                          | Validation Type | Expected Value |
|-----------------------------------------------------------------|-----------------|----------------|
| `{{response.statusCode}}`                                       | `=`             | `200`          |
| `{{`[`response.body.user.id`](http://response.body.user.id)`}}` | `=`             | `42`           |
...
| Обращение к несуществующему ключу | Проверьте структуру JSON в логах                         |
| Некорректный JSON                 | Ответ должен быть валидным (проверьте в консоли Run Log) |
...
🔎 Перед тем как указывать путь, запустите тест один раз, откройте **Run Log**, посмотрите реальный ответ и убедитесь, что путь до ключа верный.

:::

📸 *\[Вставьте скриншот: пример JSON и подсвеченный путь в логе\]*